...
export function LanguageSelector({ variant = "sidebar" }: { variant?: "sidebar" | "header" }) {
  ...
  useEffect(() => {
    // Best-effort: ask the backend which locales are live. If the
    // request fails we fall back to the canonical defaults.
    let cancelled = false;
    (async () => {
      try {
        ...
        // If the URL says one locale but the cookie says another,
        // prefer the cookie (manual override).
    ...
    // We only want this to run once on mount; changes to `value`
  ...
  function onChange(newLocale: string) {
    ...
    startTransition(async () => {
      try {
        ...
        // Reload so server components pick up the new locale cookie
        // and next-intl re-renders with the new messages.
