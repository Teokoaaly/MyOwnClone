#!/usr/bin/env python3
"""Sisyphus plan-progress verifier (M0 capa anti-olvido).
...
Salida con codigo de exito:
  0  -> todo consistente (o todo pending: caso inicial aceptable)
  1  -> inconsistencia detectada (bloquear commit / CI)

Uso:
  python scripts/check-plan-progress.py            # modo normal
  python scripts/check-plan-progress.py --strict   # exige que NINGUN task quede pending

Este script es de SOLO LECTURA: no modifica el progreso ni el arbol.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verifica el progreso del plan Sisyphus.")
    parser.add_argument("--strict", action="store_true",
                        help="Falla si queda cualquier tarea pendiente (no solo inconsistencias).")
    args = parser.parse_args()

    if not PROGRESS.exists():
        _print(f"[FAIL] No existe {PROGRESS.relative_to(ROOT)}", Colors.RED)
        return 1

    try:
        data = json.loads(PROGRESS.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        _print(f"[FAIL] {PROGRESS.relative_to(ROOT)} no es JSON valido: {exc}", Colors.RED)
        return 1
    ...
    if not tasks:
        _print("[FAIL] progress.json no tiene tareas.", Colors.RED)
        return 1

    # Validar que los IDs de task coinciden con el orden declarado.
    task_ids = [t["id"] for t in tasks]
    if order and task_ids != order:
        _print(
            f"[FAIL] El orden declarado ({order}) no coincide con los IDs de tareas "
            f"({task_ids}). Actualiza progress.json.",
            Colors.RED,
        )
        return 1
    ...
    # 1. Como maximo una tarea in_progress.
    in_progress = [t["id"] for t in tasks if t.get("status") == "in_progress"]
    if len(in_progress) > 1:
        errors.append(
            f"Hay {len(in_progress)} tareas in_progress a la vez ({in_progress}). "
    ...
    for t in tasks:
        ...
        if not sha:
            errors.append(f"{tid}: marcada 'done' sin committed_sha.")
    ...
    if errors:
        _print(f"\n{Colors.RED}{Colors.BOLD}[FAIL] {len(errors)} inconsistencia(s):", Colors.RESET)
        for e in errors:
    ...
    _print(f"\n{Colors.GREEN}[OK] progreso consistente.{Colors.RESET}")
    return 0
