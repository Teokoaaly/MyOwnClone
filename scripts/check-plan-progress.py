...
Checks `.sisyphus/progress.json` and validates:
  ...
  1. Every task marked `done` has its `evidence_file` present in git and its
     `committed_sha` exists in git.
  2. The `done` ordering respects the declared milestone order.
...
def main() -> int:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Fail if any task is not done.",
    )
    ...
    if order:
        ...
        for i, mid in enumerate(order):
            ...
            if missing:
                errors.append(
                    f"{mid} is done but predecessors are not: {missing}. Order is {order}."
                )
    ...
    for task in tasks:
        ...
        if not ev:
            errors.append(f"{tid}: done without evidence_file.")
            continue
        ...
        if not sha or sha == "PENDING":
            errors.append(f"{tid}: done with empty/PENDING committed_sha.")
    ...
    if errors:
        ...
        _print(
            "\nResolution: mark tasks done only after committing evidence and recording the real SHA.",
            Colors.YELLOW,
        )
        return 1
