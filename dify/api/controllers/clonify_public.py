...
Pregunta del usuario: {message}"""
    ...
    def generate():
        try:
            ...
            yield f"data: {json.dumps({'content': '', '***REMOVED***': True, 'context_found': result.found, 'silo': silo.value, 'confidence': round(result.scores[0], 2) if result.scores else 0, 'sources': [{'content': c[:300], 'score': round(s, 2)} for c, s in zip(result.contents, result.scores)]})}\n\n"
            yield "data: [DONE]\n\n"
