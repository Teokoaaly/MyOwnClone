...
class DatasetService:
    ...
    def _update_internal_dataset(dataset, data, user):
      ...
      ***REMOVED***ltered_data = {k: v for k, v in data.items() if v is not None or k == "description"}
        ...
        # Add metadata fields
      ***REMOVED***ltered_data["updated_by"] = user.id
      ***REMOVED***ltered_data["updated_at"] = naive_utc_now()
        # update Retrieval model
        if data.get("retrieval_model"):
          ***REMOVED***ltered_data["retrieval_model"] = data["retrieval_model"]
        # update summary index setting
        if data.get("summary_index_setting"):
          ***REMOVED***ltered_data["summary_index_setting"] = data.get("summary_index_setting")
        # update icon info
        if data.get("icon_info"):
          ***REMOVED***ltered_data["icon_info"] = data.get("icon_info")
    ...
    def _handle_indexing_technique_change(dataset, data, filtered_data):
        ...
        Args:
            ...
            data: Update data dictionary
          ***REMOVED***ltered_data: Filtered update data
        ...
        if dataset.indexing_technique != data["indexing_technique"]:
            if data["indexing_technique"] == IndexTechniqueType.ECONOMY:
              ...
              ***REMOVED***ltered_data["embedding_model"] = None
              ***REMOVED***ltered_data["embedding_model_provider"] = None
              ***REMOVED***ltered_data["collection_binding_id"] = None
                return "remove"
    ...
    def _configure_embedding_model_for_high_quality(data, filtered_data):
        ...
        Args:
            data: Update data dictionary
          ***REMOVED***ltered_data: Filtered update data to modify
        """
        ...
        try:
          ...
          ***REMOVED***ltered_data["embedding_model"] = embedding_model_name
          ***REMOVED***ltered_data["embedding_model_provider"] = embedding_model.provider
            dataset_collection_binding = DatasetCollectionBindingService.get_dataset_collection_binding(
                embedding_model.provider,
                embedding_model_name,
            )
          ***REMOVED***ltered_data["collection_binding_id"] = dataset_collection_binding.id
        except LLMBadRequestError:
            raise ValueError(
                "No Embedding Model available. Please configure a valid provider in the Settings -> Model Provider."
            )
    ...
    def _preserve_existing_embedding_settings(dataset, filtered_data):
        ...
        if dataset.embedding_model_provider and dataset.embedding_model:
          ***REMOVED***ltered_data["embedding_model_provider"] = dataset.embedding_model_provider
          ***REMOVED***ltered_data["embedding_model"] = dataset.embedding_model
            # If collection_binding_id exists, keep it too
            if dataset.collection_binding_id:
              ***REMOVED***ltered_data["collection_binding_id"] = dataset.collection_binding_id
...
class DocumentService:
    ...
    def apply_display_status_filter(cls, query, status: str | None):
      ***REMOVED***lters = cls.build_display_status_filters(status)
        if not filters:
            return query
    ...
    def get_document_file_detail(file_id: str):
      ***REMOVED***le_detail = db.session.get(UploadFile, file_id)
        return file_detail
        ...
        if document.data_source_type == DataSourceType.UPLOAD_FILE:
            if document.data_source_info:
                ...
                if data_source_info and "upload_file_id" in data_source_info:
                  ***REMOVED***le_id = data_source_info["upload_file_id"]
              ...
              ***REMOVED***:
                ...
                if not dataset.retrieval_model:
                    ...
                    dataset.retrieval_model = (
                      ...
                      ***REMOVED*** default_retrieval_model
                    )
            ...
            try:
                with redis_client.lock(lock_name, timeout=600):
                    ...
                    if knowledge_config.data_source.info_list.data_source_type == "upload_file":
                      ...
                      ***REMOVED***le_names = [file.name for file in files]
                        db_documents = list(
                            db.session.scalars(
                                select(Document).where(
