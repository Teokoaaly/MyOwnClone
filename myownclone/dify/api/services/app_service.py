...
class AppService:
    ...
    def get_paginate_apps(self, user_id: str, tenant_id: str, params: AppListParams) -> Pagination | None:
        ...
        :return:
        """
      ***REMOVED***lters = [App.tenant_id == tenant_id, App.is_universal == False]

        if params.mode == "workflow":
          ***REMOVED***lters.append(App.mode == AppMode.WORKFLOW)
        elif params.mode == "completion":
          ***REMOVED***lters.append(App.mode == AppMode.COMPLETION)
        elif params.mode == "chat":
          ***REMOVED***lters.append(App.mode == AppMode.CHAT)
        elif params.mode == "advanced-chat":
          ***REMOVED***lters.append(App.mode == AppMode.ADVANCED_CHAT)
        elif params.mode == "agent-chat":
          ***REMOVED***lters.append(App.mode == AppMode.AGENT_CHAT)
        elif params.mode == "agent":
          ***REMOVED***lters.append(App.mode == AppMode.AGENT)

        if params.status:
          ***REMOVED***lters.append(App.status == params.status)
        ...
        if params.openapi_visible:
          ***REMOVED***lters.append(App.enable_api.is_(True))
        if params.is_created_by_me:
          ***REMOVED***lters.append(App.created_by == user_id)
        if params.name:
            from libs.helper import escape_like_pattern

            name = params.name[:30]
            escaped_name = escape_like_pattern(name)
          ***REMOVED***lters.append(App.name.ilike(f"%{escaped_name}%", escape="\\"))
        if params.tag_ids and len(params.tag_ids) > 0:
            ...
            if target_ids and len(target_ids) > 0:
              ***REMOVED***lters.append(App.id.in_(target_ids))
          ***REMOVED***:
                return None
