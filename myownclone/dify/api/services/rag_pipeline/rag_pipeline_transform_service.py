...
class RagPipelineTransformService:
    ...
    def _deal_file_extensions(self, node: dict[str, Any]):
      ***REMOVED***le_extensions = node.get("data", {}).get("fileExtensions", [])
    ...
    def _deal_document_data(self, dataset: Dataset):
      ***REMOVED***le_node_id = "1752479895761"
        notion_node_id = "1752489759475"
        jina_node_id = "1752491761974"
      ***REMOVED***recrawl_node_id = "1752565402678"
        ...
        for document in documents:
            ...
            if document.data_source_type == DataSourceType.UPLOAD_FILE:
              ...
              ***REMOVED***le_id = data_source_info_dict.get("upload_file_id")
                if file_id:
                  ***REMOVED***le = db.session.get(UploadFile, file_id)
                    if file:
