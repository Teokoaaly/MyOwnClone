...
class WebhookService:
    ...
    def _extract_multipart_body(cls, webhook_trigger: WorkflowWebhookTrigger) -> tuple[dict[str, Any], dict[str, Any]]:
      ...
      ***REMOVED***les = cls._process_file_uploads(request.files, webhook_trigger) if request.files else {}
        return body, files
    ...
    def _extract_octet_stream_body(
        ...
        """
        try:
          ***REMOVED***le_content = request.get_data()
            if file_content:
                mimetype = cls._detect_binary_mimetype(file_content)
              ***REMOVED***le_obj = cls._create_file_from_binary(file_content, mimetype, webhook_trigger)
    ...
    def _process_file_uploads(
        ...
        Args:
          ***REMOVED***les: Flask request files object containing uploaded files
        ...
        for name, file in files.items():
            if file and file.filename:
                try:
                  ***REMOVED***le_content = file.stream.read()
    ...
    def _create_file_from_binary(
        ...
        Args:
          ***REMOVED***le_content: The binary content of the file
        ...
        tool_file = tool_file_manager.create_file_by_raw(
          ...
          ***REMOVED***le_binary=file_content,
            mimetype=mimetype,
        )
