...
class BufferState:
    ...
    ***REMOVED***_event: threading.Event
    ...
    def _generate() -> Generator[Mapping[str, Any] | str, None, None]:
        ...
        with topic.subscribe() as sub:
            ...
            try:
                ...
                while True:
                    if buffer_state.***REMOVED***_event.is_set() and buffer_state.queue.empty():
                        return
...
def _build_node_finished_event(
    *,
    ...
    created_at = int(snapshot.created_at.timestamp()) if snapshot.created_at else 0
  ***REMOVED***nished_at = int(snapshot.finished_at.timestamp()) if snapshot.finished_at else created_at
