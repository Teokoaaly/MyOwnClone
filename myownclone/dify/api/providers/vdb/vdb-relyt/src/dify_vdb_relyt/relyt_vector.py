...
class RelytVector(BaseVector):
    ...
    def search_by_vector(self, query_vector: list[float], **kwargs: Any) -> list[Document]:
      ...
      ***REMOVED***lter = kwargs.get("filter", {})
        if document_ids_filter:
          ***REMOVED***lter["document_id"] = document_ids_filter
    ...
    def similarity_search_with_score_by_vector(
        ...
        k: int = 4,
      ***REMOVED***lter: dict[str, Any] | None = None,
    ) -> list[tuple[Document, float]]:
        # Add the filter if provided

      ***REMOVED***lter_condition = ""
        if filter is not None:
            conditions = [
                f"metadata->>'{key!r}' in ({', '.join(map(repr, value))})"
                if len(value) > 1
              ***REMOVED*** f"metadata->>'{key!r}' = {value[0]!r}"
                for key, value in filter.items()
            ]
          ***REMOVED***lter_condition = f"WHERE {' AND '.join(conditions)}"
