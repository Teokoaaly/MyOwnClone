...
class TencentVector(BaseVector):
  ***REMOVED***eld_id: str = "id"
  ***REMOVED***eld_vector: str = "vector"
  ***REMOVED***eld_text: str = "text"
  ***REMOVED***eld_metadata: str = "metadata"
    ...
    def _create_collection(self, dimension: int):
        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            index_sparse_vector = vdb_index.SparseIndex(
                name="sparse_vector",
              ***REMOVED***eld_type=enum.FieldType.SparseVector,
    ...
    def delete_by_metadata_field(self, key: str, value: str):
        self._client.delete(
            database_name=self._client_config.database,
            collection_name=self.collection_name,
          ***REMOVED***lter=Filter(Filter.In(f"metadata.{key}", [value])),
        )
    ...
    def search_by_vector(self, query_vector: list[float], **kwargs: Any) -> list[Document]:
      ...
      ***REMOVED***lter = None
        if document_ids_filter:
          ***REMOVED***lter = Filter(Filter.In("metadata.document_id", document_ids_filter))
    ...
    def search_by_full_text(self, query: str, **kwargs: Any) -> list[Document]:
      ...
      ***REMOVED***lter = None
        ...
        res = self._client.hybrid_search(
            ...
            ann=[
                AnnSearch(
                  ***REMOVED***eld_name="vector",
                    data=[0.0] * self._dimension,
                )
            ],
            match=[
                KeywordSearch(
                  ***REMOVED***eld_name="sparse_vector",
                    data=bm25.encode_queries(query),
                ),
            ],
            rerank=WeightedRerank(
              ***REMOVED***eld_list=["vector", "sparse_vector"],
                weight=[0, 1],
            ),
            retrieve_vector=False,
            limit=kwargs.get("top_k", 4),
