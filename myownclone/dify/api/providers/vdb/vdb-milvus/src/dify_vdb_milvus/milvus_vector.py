...
class MilvusVector(BaseVector):
    ...
    def _load_collection_fields(self, fields: list[str] | None = None):
        if fields is None:
          ...
          ***REMOVED***elds = [field["name"] for field in collection_info["fields"]]
    ...
    def search_by_vector(self, query_vector: list[float], **kwargs: Any) -> list[Document]:
      ...
      ***REMOVED***lter = ""
        if document_ids_filter:
          ...
          ***REMOVED***lter = f'metadata["document_id"] in [{document_ids}]'
    ...
    def create_collection(
        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            if not self._client.has_collection(self._collection_name):
              ...
              ***REMOVED***elds = []
                if metadatas:
                  ***REMOVED***elds.append(FieldSchema(Field.METADATA_KEY, DataType.JSON, max_length=65_535))
              ...
              ***REMOVED***elds.append(FieldSchema(Field.CONTENT_KEY, DataType.VARCHAR, **content_field_kwargs))

                # Create the primary key field
              ***REMOVED***elds.append(FieldSchema(Field.PRIMARY_KEY, DataType.INT64, is_primary=True, auto_id=True))
                # Create the vector field, supports binary or float vectors
              ***REMOVED***elds.append(FieldSchema(Field.VECTOR, infer_dtype_bydata(embeddings[0]), dim=dim))
                # Create Sparse Vector Index for the collection
                if self._hybrid_search_enabled:
                  ***REMOVED***elds.append(FieldSchema(Field.SPARSE_VECTOR, DataType.SPARSE_FLOAT_VECTOR))
                ...
                if self._hybrid_search_enabled:
                    index_params_obj.add_index(
                      ***REMOVED***eld_name=Field.SPARSE_VECTOR, index_type="AUTOINDEX", metric_type="BM25"
                    )
