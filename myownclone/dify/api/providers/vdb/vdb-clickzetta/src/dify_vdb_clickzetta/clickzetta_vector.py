...
class ClickzettaVector(BaseVector):
    ...
    def _parse_metadata(self, raw_metadata: str, row_id: str):
        ...
        try:
            if raw_metadata:
              ...
              ***REMOVED***rst_pass = json.loads(raw_metadata)
        ...
        while not cls._shutdown:
            try:
                ...
                if cls._write_queue is not None:
                  ...
                  ***REMOVED***nally:
                        cls._write_queue.task_***REMOVED***()
              ***REMOVED***:
                    break
            except queue.Empty:
                continue
    ...
    def search_by_vector(self, query_vector: list[float], **kwargs: Any) -> list[Document]:
        ...
        # Build filter clause
      ***REMOVED***lter_clauses = []
        if document_ids_filter:
            safe_doc_ids = [str(id).replace("'", "''") for id in document_ids_filter]
            doc_ids_str = ",".join(f"'{id}'" for id in safe_doc_ids)
            # Use json_extract_string function for ClickZetta compatibility
          ***REMOVED***lter_clauses.append(f"json_extract_string({Field.METADATA_KEY}, '$.document_id') IN ({doc_ids_str})")
        ...
        if self._config.vector_distance_function == "cosine_distance":
            ...
            if score_threshold > 0:
              ...
              ***REMOVED***lter_clauses.append(f"{distance_func}({Field.VECTOR}, {query_vector_str}) < {2 - score_threshold}")
      ***REMOVED***:
            # For L2 distance, smaller is better
            distance_func = "L2_DISTANCE"
            if score_threshold > 0:
                query_vector_str = f"CAST('[{self._format_vector_simple(query_vector)}]' AS VECTOR({vector_dimension}))"
              ***REMOVED***lter_clauses.append(f"{distance_func}({Field.VECTOR}, {query_vector_str}) < {score_threshold}")
    ...
    def search_by_full_text(self, query: str, **kwargs: Any) -> list[Document]:
      ...
      ***REMOVED***lter_clauses.append(f"MATCH_ALL({Field.CONTENT_KEY}, '{escaped_query}')")
                    ...
                    for row in results:
                        ...
                        try:
                            if row[2]:
                              ...
                              ***REMOVED***rst_pass = json.loads(row[2])
    ...
    def _search_by_like(self, query: str, **kwargs: Any) -> list[Document]:
      ...
      ***REMOVED***lter_clauses.append(f"{Field.CONTENT_KEY} LIKE '%{escaped_query}%' ESCAPE '\\\\'")
        where_clause = " AND ".join(filter_clauses)
