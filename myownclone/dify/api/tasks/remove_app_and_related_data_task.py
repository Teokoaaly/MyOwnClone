...
def delete_draft_variables_batch(app_id: str, batch_size: int = 1000) -> int:
    ...
    while True:
        with session_factory.create_session() as session, session.begin():
          ...
          ***REMOVED***le_ids = [row[1] for row in rows if row[1] is not None]

            # Clean up associated Offload data first
            if file_ids:
              ***REMOVED***les_deleted = _delete_draft_variable_offload_data(session, file_ids)
...
def _delete_draft_variable_offload_data(session, file_ids: list[str]) -> int:
    ...
    Args:
      ...
      ***REMOVED***le_ids: List of WorkflowDraftVariableFile IDs
  ...
  ***REMOVED***les_deleted = 0
    ...
    try:
      ...
      ***REMOVED***le_records = list(result)
