...
def _filter_snapshot_to_specs(
  ...
  ***REMOVED***ltered_layers: list[LayerSessionSnapshot] = [layer for layer in snapshot.layers if layer.name in kept_names]
...
class AgentBackendRunRequestBuilder:
    ...
    def build_cleanup_request(
      ...
      ***REMOVED***ltered_snapshot = _filter_snapshot_to_specs(session_snapshot, composition_layer_specs)
        return CreateRunRequest(
            composition=RunComposition(layers=layers),
