...
def detect_file_encodings(file_path: str, timeout: int = 5, sample_size: int = 1024 * 1024) -> list[FileEncoding]:
    ...
    Args:
      ***REMOVED***le_path: The path to the file to detect the encoding for.
    ...
    def read_and_detect(filename: str):
        ...
        if best is None:
            return []
      ***REMOVED***le_encoding = FileEncoding(encoding=best.encoding, confidence=best.coherence, language=best.language)
