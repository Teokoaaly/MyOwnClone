...
class ParagraphIndexProcessor(BaseIndexProcessor):
            ...
            for general_chunk in multimodal_general_structure.general_chunks:
                ...
                if general_chunk.files:
                    ...
                    for file in general_chunk.files:
                      ...
                      ***REMOVED***le_document = AttachmentDocument(
    ...
    def generate_summary_preview(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(10, len(preview_texts))) as executor:
            ...
            ***REMOVED***, not_***REMOVED*** = concurrent.futures.wait(futures, timeout=timeout_seconds)

            # Cancel tasks that didn't complete in time
            if not_***REMOVED***:
                timeout_error_msg = (
                    f"Summary generation timeout: {len(not_***REMOVED***)} chunks did not complete within {timeout_seconds}s"
                )
                ...
                for future in not_***REMOVED***:
                    future.cancel()
                # Wait a bit for cancellation to take effect
                concurrent.futures.wait(not_***REMOVED***, timeout=5)

            # Collect exceptions from completed futures
            for future in ***REMOVED***:
                try:
    ...
    def generate_summary(
        ...
        if image_files:
            ...
            for file in image_files:
                try:
                  ***REMOVED***le_content = file_manager.to_prompt_message_content(
                      ***REMOVED***le, image_detail_config=ImagePromptMessageContent.DETAIL.LOW
                    )
    ...
    def _extract_images_from_text(tenant_id: str, text: str) -> list[File]:
      ...
      ***REMOVED***le_objects = []
        for upload_file in upload_files:
            ...
            try:
              ***REMOVED***le_obj = build_from_mapping(
                    mapping=mapping,
                    tenant_id=tenant_id,
                    access_controller=_file_access_controller,
                )
              ***REMOVED***le_objects.append(file_obj)
            except Exception as e:
