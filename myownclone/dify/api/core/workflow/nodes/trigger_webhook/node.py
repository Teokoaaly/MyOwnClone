...
class TriggerWebhookNode(Node[WebhookData]):
    ...
    def generate_file_var(self, param_name: str, file: dict[str, Any]):
      ***REMOVED***le_id = resolve_file_record_id(file.get("reference") or file.get("related_id"))
        ...
        if transfer_method_value:
            ...
            match transfer_method:
                case FileTransferMethod.LOCAL_FILE | FileTransferMethod.REMOTE_URL:
                  ***REMOVED***le["upload_file_id"] = file_id
                case FileTransferMethod.TOOL_FILE:
                  ***REMOVED***le["tool_file_id"] = file_id
                case FileTransferMethod.DATASOURCE_FILE:
                  ***REMOVED***le["datasource_file_id"] = file_id

            try:
              ***REMOVED***le_obj = self._file_reference_factory.build_from_mapping(mapping=file)
              ***REMOVED***le_segment = build_segment_with_type(SegmentType.FILE, file_obj)
    ...
    def _extract_configured_outputs(self, webhook_inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        for body_param in self.node_data.body:
            ...
            if self.node_data.content_type == ContentType.TEXT:
              ...
              ***REMOVED***le_var = self.generate_file_var(param_name, raw_data)
                if file_var:
            ...
            match param_type:
                case SegmentType.FILE:
                  ...
                  ***REMOVED***les = webhook_data.get("files", {})
                    if files and isinstance(files, dict):
                      ***REMOVED***le = files.get(param_name)
                        if file and isinstance(file, dict):
                          ***REMOVED***le_var = self.generate_file_var(param_name, file)
                            if file_var:
                                outputs[param_name] = file_var
                          ***REMOVED***:
