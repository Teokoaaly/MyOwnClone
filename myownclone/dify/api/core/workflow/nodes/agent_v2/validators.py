...
class WorkflowAgentNodeValidator:
    ...
    def validate_node_job(
        ...
        for output in node_job.declared_outputs:
            ...
            if output.check is not None and output.check.enabled and output.check.benchmark_file_ref is not None:
                cls._validate_file_ref(
                    session=session,
                    binding=binding,
                  ***REMOVED***le_ref=output.check.benchmark_file_ref,
      ...
      ***REMOVED***le_refs = node_job.metadata.get("file_refs")
    ...
    def _validate_file_ref(
        *,
        session: Session,
        binding: WorkflowAgentNodeBinding,
      ***REMOVED***le_ref: Mapping[str, Any],
        ref_context: str,
    ) -> None:
        ...
        upload_file_id = (
          ***REMOVED***le_ref.get("upload_file_id") or file_ref.get("file_id") or file_ref.get("id") or file_ref.get("reference")
        )
