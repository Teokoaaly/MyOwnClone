...
class CompletionAppGenerator(MessageBasedAppGenerator):
    ...
    def generate(
        ...
        with self._bind_file_access_scope(tenant_id=app_model.tenant_id, user=user, invoke_from=invoke_from):
          ***REMOVED***les = args["files"] if args.get("files") else []
          ***REMOVED***le_extra_config = FileUploadConfigManager.convert(
          ...
          ***REMOVED***:
            ...
            application_generate_entity = CompletionAppGenerateEntity(
              ...
              ***REMOVED***le_upload_config=file_extra_config,
                inputs=self._prepare_user_inputs(
                    user_inputs=inputs, variables=app_config.variables, tenant_id=app_model.tenant_id
                ),
                query=query,
              ***REMOVED***les=list(file_objs),
                user_id=user.id,
                stream=streaming,
    ...
    def generate_more_like_this(
        ...
        with self._bind_file_access_scope(tenant_id=app_model.tenant_id, user=user, invoke_from=invoke_from):
            # parse files
          ***REMOVED***le_extra_config = FileUploadConfigManager.convert(override_model_config_dict)
            if file_extra_config:
              ***REMOVED***le_objs = file_factory.build_from_mappings(
