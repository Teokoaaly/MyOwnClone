...
class WorkflowTool(Tool):
      ...
      ***REMOVED***les = []
        for parameter in parameter_rules:
            if parameter.type == ToolParameter.ToolParameterType.SYSTEM_FILES:
              ***REMOVED***le = tool_parameters.get(parameter.name)
                if file:
                    try:
                      ***REMOVED***le_var_list = [
                            build_file_from_stored_mapping(
                              ***REMOVED***le_mapping=cast(Mapping[str, Any], f),
                        ...
                        for file in file_var_list:
                          ***REMOVED***le_dict: dict[str, str | None] = {
                                "transfer_method": file.transfer_method.value,
                                "type": file.type.value,
                            }
                            match file.transfer_method:
                                case FileTransferMethod.TOOL_FILE:
                                  ***REMOVED***le_dict["tool_file_id"] = resolve_file_record_id(file.reference)
                                case FileTransferMethod.LOCAL_FILE:
                                  ***REMOVED***le_dict["upload_file_id"] = resolve_file_record_id(file.reference)
                                case FileTransferMethod.DATASOURCE_FILE:
                                  ***REMOVED***le_dict["datasource_file_id"] = resolve_file_record_id(file.reference)
                                case FileTransferMethod.REMOTE_URL:
                                  ***REMOVED***le_dict["url"] = file.generate_url()

                          ***REMOVED***les.append(file_dict)
                    except Exception:
    ...
    def _update_file_mapping(self, file_dict: dict[str, Any]) -> dict[str, Any]:
      ***REMOVED***le_id = resolve_file_record_id(file_dict.get("reference") or file_dict.get("related_id"))
        ...
        match transfer_method:
            case FileTransferMethod.TOOL_FILE:
              ***REMOVED***le_dict["tool_file_id"] = file_id
            case FileTransferMethod.LOCAL_FILE:
              ***REMOVED***le_dict["upload_file_id"] = file_id
