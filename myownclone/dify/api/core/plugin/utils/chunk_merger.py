...
def merge_blob_chunks[T: ToolInvokeMessage | AgentInvokeMessage](
    ...
    """
  ***REMOVED***les: dict[str, FileChunk] = {}
    ...
    for resp in response:
        if resp.type == ToolInvokeMessage.MessageType.BLOB_CHUNK:
            ...
            if chunk_id not in files:
              ***REMOVED***les[chunk_id] = FileChunk(total_length)
          ...
          ***REMOVED***les[chunk_id].data[files[chunk_id].bytes_written : files[chunk_id].bytes_written + len(blob_data)] = (
                blob_data
            )
          ***REMOVED***les[chunk_id].bytes_written += len(blob_data)
