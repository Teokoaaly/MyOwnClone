...
class Document(Base):
    ...
    # parsing
  ***REMOVED***le_id = mapped_column(LongText, nullable=True)
    ...
    def data_source_detail_dict(self) -> dict[str, Any]:
        if self.data_source_info:
            if self.data_source_type == "upload_file":
                data_source_info_dict: dict[str, Any] = json.loads(self.data_source_info)
              ***REMOVED***le_detail = db.session.scalar(
    ...
    def from_dict(cls, data: dict[str, Any]):
        return cls(
          ...
          ***REMOVED***le_id=data.get("file_id"),
...
class DocumentSegment(TypeBase):
    ...
    def get_sign_content(self) -> str:
        ...
        for match in matches:
            upload_file_id = match.group(1)
          ***REMOVED***le_extension = match.group(2)
            nonce = os.urandom(16).hex()
            timestamp = str(int(time.time()))
            data_to_sign = f"file-preview|{upload_file_id}|{timestamp}|{nonce}"
            secret_key = dify_config.SECRET_KEY.encode()
            ...
            if isinstance(queries, list):
                for query in queries:
                    if query["content_type"] == QueryType.IMAGE_QUERY:
                      ***REMOVED***le_info = db.session.scalar(select(UploadFile).where(UploadFile.id == query["content"]))
                        if file_info:
                            query["file_info"] = {
                                "id": file_info.id,
                                "name": file_info.name,
...
class DatasetKeywordTable(TypeBase):
    ...
    def keyword_table_dict(self) -> dict[str, set[Any]] | None:
      ...
      ***REMOVED***:
          ***REMOVED***le_key = "keyword_files/" + dataset.tenant_id + "/" + self.dataset_id + ".txt"
            try:
