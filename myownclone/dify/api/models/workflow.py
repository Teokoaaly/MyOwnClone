...
class Workflow(Base):  # bug
    ...
    def features(self) -> str:
      ...
      ***REMOVED***le_upload_payload = original_dict.get("file_upload")
        if not isinstance(file_upload_payload, dict):
            return self._features
      ***REMOVED***le_upload = cast(dict[str, Any], file_upload_payload)
...
class WorkflowRunDict(TypedDict):
    ...
    created_by: str
    created_at: datetime
  ***REMOVED***nished_at: datetime | None
...
class WorkflowRun(Base):
    ...
    created_by: Mapped[str] = mapped_column(StringUUID, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, server_default=func.current_timestamp())
  ***REMOVED***nished_at: Mapped[datetime | None] = mapped_column(DateTime)
    ...
    def to_dict(self) -> WorkflowRunDict:
        return WorkflowRunDict(
          ...
          ***REMOVED***nished_at=self.finished_at,
    ...
    def from_dict(cls, data: dict[str, Any]) -> "WorkflowRun":
        return cls(
          ...
          ***REMOVED***nished_at=data.get("finished_at"),
...
class WorkflowNodeExecutionModel(Base):  # This model is expected to have `offload_data` preloaded in most cases.
  ...
  ***REMOVED***nished_at: Mapped[datetime | None] = mapped_column(DateTime)
    ...
    def _load_full_content(session: orm.Session, file_id: str, storage: Storage):
      ...
      ***REMOVED***le = session.scalars(stmt).first()
...
class WorkflowNodeExecutionOffload(Base):
  ...
  ***REMOVED***le_id: Mapped[str] = mapped_column(StringUUID, nullable=False)
  ...
  ***REMOVED***le: Mapped[Optional["UploadFile"]] = orm.relationship(
        UploadFile,
...
class WorkflowDraftVariable(Base):
  ...
  ***REMOVED***le_id: Mapped[str | None] = mapped_column(
        StringUUID,
        nullable=True,
        default=None,
        ...
        match value:
            case dict():
                ...
                return build_file_from_stored_mapping(
                  ***REMOVED***le_mapping=cast(dict[str, Any], value),
                    tenant_id=tenant_id,
                )
            case list() if value:
                value_list = cast(list[Any], value)
              ***REMOVED***rst: Any = value_list[0]
                if not maybe_file_object(first):
                    return cast(Any, value)
                tenant_id = _resolve_workflow_app_tenant_id(self.app_id)
              ***REMOVED***le_list: list[File] = []
                for item in value_list:
                  ***REMOVED***le_list.append(
                        build_file_from_stored_mapping(
                          ***REMOVED***le_mapping=cast(dict[str, Any], item),
                            tenant_id=tenant_id,
                        )
                    )
        ...
        match segment_type:
            case SegmentType.FILE:
                match value:
                    ...
                    case dict():
                      ***REMOVED***le = self._rebuild_file_types(value)
            ...
            case SegmentType.ARRAY_FILE:
              ...
              ***REMOVED***le_list = self._rebuild_file_types(value)
            ...
            case list() if value:
              ...
              ***REMOVED***le_list: list[File] = []
                for item in value_list:
                    ...
                    normalized_file.pop("tenant_id", None)
                  ***REMOVED***le_list.append(build_file_from_mapping_without_lookup(file_mapping=normalized_file))
                return cast(Any, file_list)
            case _:
                return cast(Any, value)
            ...
            case SegmentType.FILE:
                match value:
                    case File():
                        return build_segment_with_type(segment_type, value)
                    case dict():
                      ***REMOVED***le = cls.rebuild_file_types(value)
                        return build_segment_with_type(segment_type, file)
                    case _:
                        raise TypeMismatchError(f"expected dict or File for FileSegment, got {type(value)}")
            case SegmentType.ARRAY_FILE:
                if not isinstance(value, list):
                    raise TypeMismatchError(f"expected list for ArrayFileSegment, got {type(value)}")
              ***REMOVED***le_list = cls.rebuild_file_types(value)
                return build_segment_with_type(segment_type=segment_type, value=file_list)
            case _:
                return build_segment_with_type(segment_type=segment_type, value=value)
