...
def resolve_file_mapping_tenant_id(
    *,
  ***REMOVED***le_mapping: Mapping[str, Any],
...
def build_file_from_mapping_without_lookup(*, file_mapping: Mapping[str, Any]) -> File:
  ...
  ***REMOVED***le_id = mapping.get("file_id")
    if not isinstance(file_id, str) or not file_id:
        legacy_id = mapping.get("id")
      ***REMOVED***le_id = legacy_id if isinstance(legacy_id, str) and legacy_id else None
  ...
  ***REMOVED***lename = mapping.get("filename")
    ...
    return File(
      ***REMOVED***le_id=file_id,
        tenant_id=tenant_id,
      ***REMOVED***le_type=_coerce_file_type(mapping.get("file_type", mapping.get("type"))),
...
def build_file_from_stored_mapping(
  ...
  ***REMOVED***les through the workflow factory instead of trusting serialized metadata.
  ...
  ***REMOVED***le_mapping: Mapping[str, Any],
    ...
    if transfer_method == FileTransferMethod.REMOTE_URL and record_id is None:
        return build_file_from_stored_mapping(
          ***REMOVED***le_mapping=file_mapping,
            tenant_id="",
        )
