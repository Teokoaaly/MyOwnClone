...
class TestPluginModelRuntime:
    ...
    def test_fetch_model_providers_uses_tenant_ttl_cache_across_runtime_instances(
        ...
        monkeypatch.setattr(plugin_service_module.dify_config, "PLUGIN_MODEL_PROVIDERS_CACHE_TTL", 300)
      ***REMOVED***rst_client = Mock(spec=PluginModelClient)
      ***REMOVED***rst_client.fetch_model_providers.return_value = [_build_plugin_model_provider(tenant_id="tenant")]
        second_client = Mock(spec=PluginModelClient)
      ***REMOVED***rst_runtime = PluginModelRuntime(
      ...
      ***REMOVED***rst_providers = first_runtime.fetch_model_providers()
      ...
      ***REMOVED***rst_client.fetch_model_providers.assert_called_once_with("tenant")
    ...
    def test_fetch_model_providers_cache_is_tenant_isolated(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(plugin_service_module.dify_config, "PLUGIN_MODEL_PROVIDERS_CACHE_TTL", 300)
      ***REMOVED***rst_client = Mock(spec=PluginModelClient)
      ***REMOVED***rst_client.fetch_model_providers.return_value = [_build_plugin_model_provider(tenant_id="tenant-a")]
        second_client = Mock(spec=PluginModelClient)
      ...
      ***REMOVED***rst_providers = first_runtime.fetch_model_providers()
        ...
        assert [provider.provider for provider in first_providers] == ["langgenius/openai/openai"]
        assert [provider.provider for provider in second_providers] == ["langgenius/openai/openai"]
      ***REMOVED***rst_client.fetch_model_providers.assert_called_once_with("tenant-a")
...
def test_get_schema_cache_key_is_stable_across_credential_order() -> None:
  ...
  ***REMOVED***rst = runtime._get_schema_cache_key(
        provider="langgenius/openai/openai",
        model_type=ModelType.LLM,
        model="gpt-4o-mini",
        credentials={"b": "2", "a": "1"},
...
def test_get_schema_cache_key_separates_distinct_user_scopes() -> None:
  ...
  ***REMOVED***rst = first_runtime._get_schema_cache_key(
        provider="langgenius/openai/openai",
        model_type=ModelType.LLM,
        model="gpt-4o-mini",
        credentials={"a": "1"},
