...
class TestBlob:
    ...
    def test_source_property_and_repr_include_path(self, tmp_path):
      ***REMOVED***le_path = tmp_path / "sample.txt"
      ***REMOVED***le_path.write_text("hello", encoding="utf-8")
    ...
    def test_as_string_from_path(self, tmp_path):
      ***REMOVED***le_path = tmp_path / "sample.txt"
      ***REMOVED***le_path.write_text("from-file", encoding="utf-8")
    ...
    def test_as_bytes_from_bytes_str_and_path(self, tmp_path):
      ...
      ***REMOVED***le_path = tmp_path / "sample.bin"
      ***REMOVED***le_path.write_bytes(b"from-path")
    ...
    def test_as_bytes_io_for_bytes_and_path(self, tmp_path):
      ...
      ***REMOVED***le_path = tmp_path / "stream.bin"
      ***REMOVED***le_path.write_bytes(b"path-stream")
    ...
    def test_from_path_respects_guessing_and_explicit_mime(self, tmp_path):
      ***REMOVED***le_path = tmp_path / "example.txt"
      ***REMOVED***le_path.write_text("x", encoding="utf-8")
