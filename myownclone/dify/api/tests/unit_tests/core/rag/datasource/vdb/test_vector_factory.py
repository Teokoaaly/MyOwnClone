...
def test_lazy_embeddings_defer_real_load_until_first_embed_call(vector_factory_module, monkeypatch: pytest.MonkeyPatch):
  ...
  ***REMOVED***rst ``embed_*`` call (i.e. create / search paths) so cleanup paths
...
def test_create_multimodal_filters_missing_uploads(vector_factory_module, monkeypatch: pytest.MonkeyPatch):
  ...
  ***REMOVED***le_base64 = base64.b64encode(b"abc").decode()
...
def test_filter_duplicate_texts_and_getattr(vector_factory_module):
  ...
  ***REMOVED***ltered = vector._filter_duplicate_texts(docs)
