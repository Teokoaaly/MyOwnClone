...
def test_execute_code_returns_stdout_on_success(mocker: MockerFixture) -> None:
    ...
    response.json.return_value = {"code": 0, "message": "ok", "data": {"stdout": "***REMOVED***", "error": None}}
    client = MagicMock()
    client.post.return_value = response
    mocker.patch("core.helper.code_executor.code_executor.get_pooled_http_client", return_value=client)

    assert code_executor_module.CodeExecutor.execute_code(cast(Any, "python3"), preload="", code="print(1)") == "***REMOVED***"
