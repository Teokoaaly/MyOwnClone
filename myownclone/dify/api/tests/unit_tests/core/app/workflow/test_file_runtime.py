...
def _build_file(
    *,
    ...
    return File(
      ***REMOVED***le_id="file-id",
      ***REMOVED***le_type=FileType.IMAGE,
        transfer_method=transfer_method,
        reference=reference,
        remote_url=remote_url,
      ***REMOVED***lename="diagram.png",
        extension=extension,
        mime_type="image/png",
        size=128,
    )
...
def test_resolve_file_url_requires_file_reference() -> None:
    runtime = _build_runtime()
  ***REMOVED***le = SimpleNamespace(transfer_method=FileTransferMethod.LOCAL_FILE, reference=None)
...
def test_runtime_helper_wrappers_delegate_to_config_and_io(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch.object(
      ***REMOVED***le_runtime.remote_fetcher.graphon_remote_file_fetcher,
        "get",
