...
def _build_image_file(
  ...
  ***REMOVED***le_id: str,
    ...
    return File(
      ...
      ***REMOVED***lename=f"{file_id}{extension}",
...
class TestLLMNodeSaveMultiModalImageOutput:
    def test_llm_node_save_inline_output(self, llm_node_for_multimodal: tuple[LLMNode, LLMFileSaver]):
        ...
        mock_file = File(
          ...
          ***REMOVED***lename="test-file.png",
            extension=".png",
            mime_type="image/png",
            size=9,
        )
        mock_file_saver.save_binary_string.return_value = mock_file
      ***REMOVED***le = llm_node.save_multimodal_image_output(
            content=content,
          ***REMOVED***le_saver=mock_file_saver,
        )
...
class TestSaveMultimodalOutputAndConvertResultToMarkdown:
    ...
    def test_image_content_with_inline_data(self, llm_node_for_multimodal, monkeypatch: pytest.MonkeyPatch):
        ...
        gen = llm_node._save_multimodal_output_and_convert_result_to_markdown(
          ...
          ***REMOVED***le_saver=mock_file_saver,
          ***REMOVED***le_outputs=llm_node._file_outputs,
        )
...
def test_invoke_llm_dispatches_to_expected_model_method(structured_output_enabled, structured_output):
  ...
  ***REMOVED***le_saver = mock.MagicMock(spec=LLMFileSaver)
...
def test_handle_invoke_result_streaming_collects_text_metrics_and_structured_output():
  ...
  ***REMOVED***rst_chunk = LLMResultChunkWithStructuredOutput(
        model="gpt-3.5-turbo",
        prompt_messages=[],
  ...
  ***REMOVED***nal_chunk = LLMResultChunk(
        model="gpt-3.5-turbo",
        prompt_messages=[],
        delta=LLMResultChunkDelta(
            index=1,
