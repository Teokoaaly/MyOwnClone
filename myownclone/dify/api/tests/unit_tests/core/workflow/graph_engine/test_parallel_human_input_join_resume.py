...
def _build_graph(runtime_state: GraphRuntimeState, repo: HumanInputFormRepository) -> Graph:
    ...
    human_a = HumanInputNode(
      ...
      ***REMOVED***le_reference_factory=DifyFileReferenceFactory(graph_init_params.run_context),
...
def test_parallel_human_input_join_completes_after_second_resume() -> None:
  ...
  ***REMOVED***rst_resume_state = pause_store.load()
  ***REMOVED***rst_resume_repo = StaticRepo(
        {
            "human_a": _form(submitted=True, action_id="approve"),
            "human_b": _form(submitted=False, action_id=None),
        }
    )
  ***REMOVED***rst_resume_graph = _build_graph(first_resume_state, first_resume_repo)
  ***REMOVED***rst_resume_events = _run_graph(first_resume_graph, first_resume_state)
