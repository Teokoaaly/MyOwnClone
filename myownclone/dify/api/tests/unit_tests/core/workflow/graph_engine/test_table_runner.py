...
class WorkflowRunner:
        ...
        if fixtures_dir is None:
            ...
            for parent in current_file.parents:
                if parent.name == "api" and (parent / "tests").exists():
                  ***REMOVED***xtures_dir = parent / "tests" / "fixtures" / "workflow"
                    break
          ***REMOVED***:
    ...
    def load_fixture(self, fixture_name: str) -> dict[str, Any]:
        ...
        if not fixture_name.endswith(".yml") and not fixture_name.endswith(".yaml"):
          ***REMOVED***xture_name = f"{fixture_name}.yml"

      ***REMOVED***xture_path = self.fixtures_dir / fixture_name
        return _load_fixture(fixture_path, fixture_name)

    def create_graph_from_fixture(
        self,
      ***REMOVED***xture_data: dict[str, Any],
        query: str = "",
...
class TableTestRunner:
    ...
    def __init__(
        self,
      ***REMOVED***xtures_dir: Path | None = None,
        max_workers: int = 4,
        ...
        Args:
          ***REMOVED***xtures_dir: Directory containing fixture files
    ...
    def _create_graph_runtime_state(self, test_case: WorkflowTestCase) -> tuple[Graph, GraphRuntimeState]:
      ...
      ***REMOVED***xture_data = self.workflow_runner.load_fixture(test_case.fixture_path)

        return self.workflow_runner.create_graph_from_fixture(
          ***REMOVED***xture_data=fixture_data,
                ...
                try:
                    ...
                    if fail_fast and not result.success:
                        ...
                        for remaining_future in future_to_test:
                            if not remaining_future.***REMOVED***():
