...
def _mock_upload_dependencies(
    ...
    *,
  ***REMOVED***le_size_within_limit: bool = True,
):
  ***REMOVED***le_info = SimpleNamespace(
      ***REMOVED***lename="report.txt",
        extension=".txt",
        mimetype="text/plain",
        size=3,
    )
  ...
  ***REMOVED***le_service_cls = MagicMock()
  ***REMOVED***le_service_cls.is_file_size_within_limit.return_value = file_size_within_limit
...
def test_remote_file_upload_success_when_fetch_falls_back_to_get(app, monkeypatch: pytest.MonkeyPatch) -> None:
  ...
  ***REMOVED***le_service_cls, current_user = _mock_upload_dependencies(monkeypatch)
  ...
  ***REMOVED***le_service_cls.return_value.upload_file.return_value = upload_file
  ...
  ***REMOVED***le_service_cls.return_value.upload_file.assert_called_once_with(
      ***REMOVED***lename="report.txt",
...
def test_remote_file_upload_translates_service_file_too_large_error(app, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(remote_files_module.remote_fetcher, "make_request", make_request)
  ***REMOVED***le_service_cls, current_user = _mock_upload_dependencies(monkeypatch)
  ***REMOVED***le_service_cls.return_value.upload_file.side_effect = ServiceFileTooLargeError("size exceeded")
...
def test_remote_file_upload_translates_service_unsupported_type_error(app, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(remote_files_module.remote_fetcher, "make_request", make_request)
  ***REMOVED***le_service_cls, current_user = _mock_upload_dependencies(monkeypatch)
  ***REMOVED***le_service_cls.return_value.upload_file.side_effect = ServiceUnsupportedFileTypeError()
