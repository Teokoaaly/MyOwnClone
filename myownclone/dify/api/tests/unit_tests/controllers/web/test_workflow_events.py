...
class TestWorkflowEventsApi:
    ...
    def test_finished_run_returns_sse_response(
        ...
        run = SimpleNamespace(
            ...
            created_by_role=CreatorUserRole.END_USER,
            created_by="eu-1",
          ***REMOVED***nished_at=datetime(2024, 1, 1),
        )
        mock_repo = MagicMock()
        mock_repo.get_workflow_run_by_id_and_tenant_id.return_value = run
        mock_factory.create_api_workflow_run_repository.return_value = mock_repo

      ***REMOVED***nish_response = MagicMock()
      ***REMOVED***nish_response.model_dump.return_value = {"task_id": "run-1"}
      ***REMOVED***nish_response.event.value = "workflow_finished"
