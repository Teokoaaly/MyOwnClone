...
class BaseWorkflowRunPayload(BaseModel):
  ***REMOVED***les: list[dict[str, Any]] | None = None
...
def _parse_file(workflow: Workflow, files: list[dict] | None = None) -> Sequence[File]:
  ***REMOVED***les = files or []

  ***REMOVED***le_extra_config = FileUploadConfigManager.convert(workflow.features_dict, is_vision=False)
  ***REMOVED***le_objs: Sequence[File] = []
    if file_extra_config is None:
        return file_objs
  ***REMOVED***le_objs = file_factory.build_from_mappings(
        mappings=files,
...
class DraftWorkflowNodeRunApi(Resource):
    ...
    def post(self, app_model: App, node_id: str):
      ...
      ***REMOVED***les = _parse_file(draft_workflow, args.get("files"))
        workflow_service = WorkflowService()
...
class DefaultBlockConfigApi(Resource):
    ...
    def get(self, app_model: App, block_type: str):
      ...
      ***REMOVED***lters = None
        if args.q:
            try:
              ***REMOVED***lters = json.loads(args.q)
...
class DraftWorkflowTriggerNodeApi(Resource):
    ...
    def post(self, app_model: App, node_id: str):
      ...
      ***REMOVED***les = _parse_file(draft_workflow, raw_files if isinstance(raw_files, list) else None)
        try:
