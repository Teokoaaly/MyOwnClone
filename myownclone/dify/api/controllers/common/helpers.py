...
def guess_file_info_from_response(response: httpx.Response):
  ...
  ***REMOVED***lename = urllib.parse.unquote(os.path.basename(url_path))
    ...
    if not filename:
        ...
        if content_disposition:
          ***REMOVED***lename_match = re.search(r'filename="?(.+)"?', content_disposition)
            if filename_match:
              ***REMOVED***lename = filename_match.group(1)

    # If still no filename, generate a unique one
    if not filename:
        unique_name = str(uuid4())
      ***REMOVED***lename = f"{unique_name}"
    ...
    if not extension:
      ...
      ***REMOVED***lename = f"{filename}{extension}"
