...
def extract_filename(url_or_path: str, content_disposition: str | None) -> str | None:
    ...
    """
  ***REMOVED***lename: str | None = None
    if content_disposition:
      ***REMOVED***lename_star_match = re.search(r"filename\*=([^;]+)", content_disposition)
        if filename_star_match:
            ...
            try:
              ...
              ***REMOVED***lename = urllib.parse.unquote(value, encoding=charset, errors="replace")
            except Exception:
                if "''" in raw_star:
                  ***REMOVED***lename = urllib.parse.unquote(raw_star.split("''")[-1])
              ***REMOVED***:
                  ***REMOVED***lename = urllib.parse.unquote(raw_star)
        ...
        if not filename:
            ...
            if raw:
                if len(raw) >= 2 and raw[0] == raw[-1] == '"':
                    raw = raw[1:-1]
              ***REMOVED***lename = urllib.parse.unquote(raw)
    ...
    if not filename:
      ...
      ***REMOVED***lename = urllib.parse.unquote(candidate, errors="replace") if candidate else None

    if filename:
      ***REMOVED***lename = os.path.basename(filename)
...
def get_remote_file_info(url: str) -> tuple[str, str, int]:
  ...
  ***REMOVED***lename = os.path.basename(url_path)
    ...
    if resp.status_code == httpx.codes.OK:
        ...
        if extracted_filename:
          ***REMOVED***lename = extracted_filename
            mime_type = _guess_mime_type(filename)
      ***REMOVED***le_size = int(resp.headers.get("Content-Length", file_size))
        if not mime_type:
            mime_type = resp.headers.get("Content-Type", "").split(";")[0].strip()

    if not filename:
        extension = mimetypes.guess_extension(mime_type) or ".bin"
      ***REMOVED***lename = f"{uuid.uuid4().hex}{extension}"
        if not mime_type:
            mime_type = _guess_mime_type(filename)
