...
Creates the 14 new tables that form the MyOwnClone data layer on top of the base platform:
...
Every MyOwnClone table is scoped to a tenant so the multi-tenant boundary is preserved
without touching the base platform core models.
...
down_revision = '0000000000a0'   # → create_base_tables_tenants_accounts
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('description', LongText(), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('system_prompt', LongText(), nullable=False),
        sa.Column('is_active', sa.Boolean(), server_default=sa.text('true'), nullable=False),
        _timestamps(),
    )
    ...
    op.create_table(
        ...
        sa.Column('trigger_condition', LongText(), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('body_text', LongText(), nullable=True),
        sa.Column('body_html', LongText(), nullable=True),
        sa.Column('draft_reply', LongText(), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('name', sa.String(255), nullable=False),
        sa.Column('subject', sa.String(500), nullable=True),
        sa.Column('body', LongText(), nullable=True),
        sa.Column(
            'trigger_keywords',
    ...
    op.create_table(
        ...
        sa.Column('transcript', LongText(), nullable=True),
        sa.Column('notes', LongText(), nullable=True),
        _timestamps(),
    )
    ...
    op.create_table(
        ...
        _uuid('id', is_postgres, pk=True),
        _uuid_fk('clone_id', 'clone_configs', is_postgres, ondelete='CASCADE'),
        sa.Column('question', LongText(), nullable=True),
    ...
    op.create_table(
        ...
        _uuid_fk_account,
        _uuid_fk_tenant,
        sa.Column('reason', LongText(), nullable=True),
...
def _uuid(col_name: str, is_postgres: bool, *, pk: bool = False) -> sa.Column:
    ...
    return sa.Column(col_name, StringUUID, primary_key=pk, nullable=False)
