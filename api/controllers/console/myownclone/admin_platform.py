...
class IngestionStatusApi(Resource):
    ...
    def get(self):
        ...
        total_chunks = db.session.execute(select(func.count(Chunk.id))).scalar() or 0
        chunks_with_embedding = db.session.execute(
            select(func.count(Chunk.id)).where(Chunk.embedding.isnot(None))
        ).scalar() or 0
        
        total_sources = db.session.execute(select(func.count(Source.id))).scalar() or 0
        sources_by_status = {}
        for status in ["uploading", "processing", "ready", "failed"]:
            count = db.session.execute(
                select(func.count(Source.id)).where(Source.status == status)
            ).scalar() or 0
            sources_by_status[status] = count
        
        return {
            "total_chunks": total_chunks,
            "chunks_with_embedding": chunks_with_embedding,
            "chunks_pending_embedding": total_chunks - chunks_with_embedding,
...
class SystemStatusApi(Resource):
    """Comprehensive server monitoring with real-time metrics.

    Collects OS, database, Redis, Ollama, worker, Docker, and API metrics.
    Every value is measured live — no fake data.
    """
