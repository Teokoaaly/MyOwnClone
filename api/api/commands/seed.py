...
DEMO_CLONE_ID = "demo-clone-001"


@click.command("seed-demo-data")
def seed_demo_data():
    """Create demo data for testing MyOwnClone."""
    ...
    if not existing_meeting:
        meeting = MeetingType_(
            clone_id=DEMO_CLONE_ID,
            name="Consultation",
            duration_minutes=30,
            price_cents=9900,
    ...
    # ── Admin User ───────────────────────────────────────────────────────────
    # Note: User/Account management is handled by the base platform (models.account).
    # For demo purposes, we create a stub record in the impersonation log if needed.
    # The actual user creation would require the base platform's User model.
    click.echo("")
    click.echo("  ℹ Admin user (admin@youownclone.com / admin123)")
    click.echo("     Note: User management is part of the base platform.")
