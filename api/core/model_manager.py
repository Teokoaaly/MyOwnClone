    ...
    def invoke_for_task(
        ...
        *,
        ...
        try:
            ...
            # Add fallback model if available (chat_fallback task)
            if task.value == "chat":
                try:
                    fallback = self.registry.get_model_for_task(tenant_id=tenant_id, task=AITask.CHAT_FALLBACK)
                    if fallback and fallback.model_id != resolved.model_id:
                        fallback_adapter = self._provider_adapter_for(fallback)
                        fallback_params = self._build_generation_params(fallback)
                        candidates.append(RetryCandidate(
                            provider_name=fallback.provider,
                            model_id=fallback.model_id,
                            priority=fallback.priority + 1,
                            invoke=lambda: fallback_adapter.generate(prompt=fitted.text, params=fallback_params),
                        ))
                        logger.info("Added fallback model: %s/%s", fallback.provider, fallback.model_id)
                except Exception as exc:
                    logger.warning("Could not resolve fallback model: %s", exc)
            reply = self.retry_client.invoke(candidates)
