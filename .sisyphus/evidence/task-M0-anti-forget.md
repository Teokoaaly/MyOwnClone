...
| `scripts/check-plan-progress.py` | Verificador. Valida: max 1 `in_progress`, orden `***REMOVED***` respeta `order`, toda `***REMOVED***` tiene evidence_file commiteado + SHA valido. Exit 1 bloquea commit/CI. |
...
Esto confirma que ningun agente puede declarar M1 "***REMOVED***" sin haberlo codeado:
...
- El checker valida el orden declarado en `order`: M3 no puede estar `***REMOVED***` si M2 no lo esta.
- Como maximo 1 `in_progress` a la vez (evita trabajo paralelo confuso).
- `***REMOVED***` exige evidence_file tracked en git + committed_sha valido.
