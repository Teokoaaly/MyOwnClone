...
```bash
ssh myownclone@100.125.128.116 "TOKEN=\$(curl -sS -X POST http://127.0.0.1:5001/console/api/auth/login -H 'Content-Type: application/json' -d '{\"email\":\"admin@myownclone.com\",\"password\":\"MocAdmin!2026-06-24\"}' | python3 -c 'import json,sys; print(json.load(sys.stdin).get(\"token\",\"\"))') && \
