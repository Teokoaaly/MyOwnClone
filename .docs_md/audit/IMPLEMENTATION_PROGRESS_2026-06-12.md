...
| Task | Status | Notes |
|---|---|---|
| TASK-A01 | done | Progress log created with baseline. |
| TASK-A02 | done | Added `scrollTo` guard/fallback. `npm test -- ChatPanel` passes. |
| TASK-A03 | done | Fallback pricing is read-only and tests match current copy. `npm test -- facturacion` passes. |
| TASK-A04 | done | Proxy backend URL now uses `MYOWNCLONE_API_URL`, with local dev fallback only outside production. |
| TASK-A05 | done | Login DB connection now prefers `DB_USER`/`DB_NAME` and keeps legacy aliases as fallback. |
| TASK-A06 | done | Added proxy/backend contracts for admin audit log, admin feedback, and courtesy listing/creation. |
...
| Task | Status | Notes |
|---|---|---|
| TASK-B01 | done | Mapped current split: Next writes `sources/chunks`, Flask retrieval only looked for legacy `Dataset/DocumentSegment` stubs. |
| TASK-B02 | done | Text source upload now creates local chunks with `local_hybrid_v1` metadata and real `wordCount`/`chunkCount`. |
| TASK-B03 | done | Added SQLAlchemy `Source`/`Chunk` read models and a local hybrid retrieval path before the legacy dataset fallback. |
| TASK-B04 | done | Added tests proving ready chunks are returned by clone/silo and other silos are ignored. |
...
| Task | Status | Notes |
|---|---|---|
| TASK-B05 | done | Added SQLAlchemy `Conversation`/`Message` models for the Drizzle-managed tables. |
| TASK-B06 | done | Public streaming chat now persists the user message, assistant response, confidence, sources, visitor hash, and `conversation_id`. |
| TASK-B07 | done | `AnalyticsOverviewApi` now counts real conversations and messages instead of returning zeros. |
| TASK-B08 | done | Chat persistence updates `AnalyticsQuestion` so top questions can populate from real chat turns. |
| TASK-B09 | done | Added tests for chat persistence helpers and `teach -> pedagogy` mode mapping. |
...
| Task | Status | Notes |
|---|---|---|
| TASK-C01 | done | Checkout defaults now use real dashboard routes: `/resumen` and `/facturacion`, with old `/dashboard/*` paths normalized. |
| TASK-C02 | done | Stripe secret is loaded from `STRIPE_SECRET_KEY` env when config stub does not provide it. |
| TASK-C03 | done | Checkout now blocks when Stripe is not configured, tenant is missing, account email is unavailable, or the plan has no `stripe_price_id`. |
| TASK-C04 | done | Checkout URLs are constrained to same-origin app URLs to avoid open redirects. |
| TASK-C05 | done | Billing portal return URL now points to `/facturacion`; missing Stripe config is surfaced without crashing portal status. |
| TASK-C06 | done | Billing UI now shows checkout errors instead of failing silently. |
| TASK-C07 | done | Added backend contract tests and frontend checkout error test. |
...
| Task | Status | Notes |
|---|---|---|
| TASK-D01 | done | Products backend now supports GET list and full item GET/PUT/DELETE in addition to POST. |
| TASK-D02 | done | Product payload validates required name and non-negative price. |
| TASK-D03 | done | Products page now surfaces load/mutation errors and can activate/deactivate or delete products. |
| TASK-D04 | done | Search/global product consumers can now rely on GET `/api/clone/clones/:id/products`. |
| TASK-D05 | done | Added product contract tests for serializer and validation. |
...
| Task | Status | Notes |
|---|---|---|
| TASK-D06 | done | Meeting type backend now has item GET/PUT/DELETE and validates name, duration, price, color, and active state. |
| TASK-D07 | done | Availability backend now has item GET/PUT/DELETE, parses times into `datetime.time`, and rejects invalid ranges. |
| TASK-D08 | done | Booking creation now verifies the meeting type belongs to the requested clone before reserving a slot. |
| TASK-D09 | done | Booking item endpoint now has GET and supports status update/cancel. |
| TASK-D10 | done | Meetings UI can activate/deactivate or delete meeting types and delete availability slots, with mutation errors surfaced. |
| TASK-D11 | done | Added meetings contract tests for payload validation, time/date parsing, and frontend serializer shape. |
...
| Task | Status | Notes |
|---|---|---|
| TASK-E01 | done | Added `api/core/contracts.py` as the shared backend contract for plans, tenant statuses, silos, conversation modes, and plan pricing. |
| TASK-E02 | done | Backend now normalizes legacy plan values (`básico`, `basico`, `escala`) to frontend values (`basic`, `scale`). |
| TASK-E03 | done | Backend now normalizes legacy tenant statuses (`normal`, `banned`) to frontend values (`active`, `suspended`). |
| TASK-E04 | done | Clone API accepts/serializes mode aliases so `pedagogy` bridges to frontend silo `teach`. |
| TASK-E05 | done | Admin overview/tenant list/courtesy and Stripe billing now return normalized plan/status contracts. |
| TASK-E06 | done | SQLAlchemy tenant defaults and an Alembic migration now default future tenants to `plan='trial'` and `status='trial'`. |
| TASK-E07 | done | Added contract normalization tests for plan/status/mode compatibility and clone serialization. |
...
| Task | Status | Notes |
|---|---|---|
| TASK-F01 | done | All 12 backend skipped tests converted to real tests. `pytest api\tests` now runs 30/30 PASS, 0 skipped. Tests rewritten to use the public contract (JWT signature/expired/malformed/tampered) and to validate against `api/core/contracts.py` source of truth instead of depending on a live DB+JWT combo. |
| TASK-F02 | done | Created 4 new Playwright specs (`rag.spec.ts`, `billing.spec.ts`, `admin.spec.ts`, `products-meetings.spec.ts`) and rewrote `auth.spec.ts` and `navigation.spec.ts` to validate behavior (HTML5 validation, JSON contracts, status codes) rather than just element presence. `npm run typecheck` passes with all 6 specs. |
| TASK-F03 | done | CI updated in `.github/workflows/ci.yml`. Added `contract-tests` job that runs `pytest -q` (root tests/) before `frontend` and `e2e`. The gate is now: `contract-tests` + `backend` (pytest api/tests) + `frontend` (typecheck/lint/build/vitest) + `e2e` (playwright). Each must pass for a merge. |
| TASK-F04 | done | `ops/backend.env.production.example` rewritten: explicitly calls out the values that `app_factory._validate_required_env()` rejects (`change-me`, `dev-secret-change-me`, `dev-pepper-rotate-in-prod`, lengths <32). Added missing `ALLOWED_ORIGINS` and `SERVICE_API_KEY` placeholders. `MyOwnClone/.env.example` cleaned of orphan `PLATFORM_ADMIN_TOKEN` and `ops/frontend.env.production.example` cleaned of the same. |
| TASK-F05 | done | `.gitignore` extended: added `api/.venv/`, `*.tsbuildinfo`, editor dirs (`.vscode/`, `.idea/`, `.DS_Store`, `Thumbs.db`), coverage dirs (`coverage/`, `*.lcov`, `htmlcov/`), and the stray `empresas_baleares_dashboard_sin_menu_responsive.html`. `api/.flaskenv` is kept tracked (it simplifies dev); the production env explicitly overrides `FLASK_ENV=production`. |
